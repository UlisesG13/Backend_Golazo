from src.core.exceptions import NotFoundError, BadRequestError, ConflictError
...
class UpdatePromocion:
    ...
    async def execute(self, promocion_id: int, dto) -> PromocionModel:
        ...
        if not current.date_is_valid():
            raise BadRequestError("Error de consistencia: La fecha de inicio no puede ser posterior a la de expiración.")

        # 4. Verificar si el nuevo código (si cambió) ya está tomado por otra promoción
        existing = await self.repo.get_by_codigo(current.codigo)
        if existing and existing.promocion_id != promocion_id:
            raise ConflictError(f"El código {current.codigo} ya está siendo usado por otra promoción.")
